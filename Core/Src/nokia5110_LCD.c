 ...
 */
void LCD_init(GPIO_TypeDef *CS_GPIO, uint16_t CS_PIN, GPIO_TypeDef *DC_GPIO, uint16_t DC_PIN, SPI_HandleTypeDef SPI_CH)
{

  LCD_setCE(CS_GPIO, CS_PIN);
  LCD_setDC(DC_GPIO, DC_PIN);
  LCD_setSPICH(SPI_CH);
  ...
  LCD_write(0xBE, LCD_COMMAND);               //set LCD Vop(Contrast).
...
void LCD_drawBitmap(int x, int y, uint8_t *bitmap, int sx, int sy)
{
  int bit;
  int data;
  ...
  for (int cy = 0; cy < sy; cy++)
  {
    ...
    for (int cx = 0; cx < sx; cx++)
    {
      ...
      if ((data & (1 << bit)) > 0)
        LCD_setPixel(x + cx, y + cy, true);
      else
        LCD_setPixel(x + cx, y + cy, false);
    }
  }
